...
namespace eqdseq
{
    ...
    public static class HarmonyPatches
    {
        ...
        static HarmonyPatches()
        {
            ...
            if (LoadedModManager.RunningModsListForReading.Any(mod => mod.PackageIdPlayerFacing == "telardo.MultiFloors"))
            {
                try
                {
                    var a = AccessTools.TypeByName("eqdseq.JobGiver_IdleLovin");
                    if (a != null)
                    {
                        var m = AccessTools.Method(a, "ExMultiFloorsCanReachAcrossLevel");
                        if (m != null)
                        {
                            harmony.Patch(
                                original: m,
    ...
    public static class HarmonyPatches_Transpiler
    {
        ...
        public static IEnumerable<CodeInstruction> DoLovinWhenIdle_MultiFloors_Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            for (int i = 0; i < codes.Count; i++)
            {
                ...
                {
                    codes.Insert(i + 0, new CodeInstruction(OpCodes.Ldarg_1, null));
                    codes.Insert(i + 1, new CodeInstruction(OpCodes.Ldarg_2, null));
                    codes.Insert(i + 2, new CodeInstruction(OpCodes.Ldarg_3, null));
    ...
    public class JobGiver_IdleLovin : ThinkNode_JobGiver
    {
        ...
        public bool ExMultiFloorsCanReachAcrossLevel(Pawn p, Thing b, Map m, bool f)
        {
            return false;
        }
        public Job ExMultiFloorsJob(Pawn p, Pawn pt, Thing b, Map m, bool f)
        {
        ...
        protected override Job TryGiveJob(Pawn pawn)
        {
            ...
            {
                flagm = ExMultiFloorsCanReachAcrossLevel(pawn, ownedBed, ownedBedMap, false);
                if (!flagm)
                {
                    pawn.lastTryTick() = ticksGame + 15000;
                    return null;
            ...
            foreach (Pawn pawn2 in ownedBed.OwnersForReading)
            {
                ...
                if (flagm)
                {
                    if (!ExMultiFloorsCanReachAcrossLevel(pawn2, ownedBed, ownedBedMap, false))
                    {
                        pawn2.lastCheckTick() = ticksGame + 2500;
                ...
                if (!flagm && pawn2curJobdef == JobDefOf.Wait_Asleep && pawn2JobTag == JobTag.SatisfyingNeeds && pawn2.Position == sleepingSpot2 && pawn2.jobs.posture == PawnPosture.LayingInBed)
                {
                ...
                if (pawn2JobTag == JobTag.Idle)
                {
                    if (pawn2curJobdef == JobDefOf.GotoWander || pawn2curJobdef == JobDefOf.Wait_Wander)
                    {
                        ...
                        if (flagm)
                        {
                            return ExMultiFloorsJob(pawn, pawn2, ownedBed, ownedBedMap, false);
                        }
                        Job newJob = JobMaker.MakeJob(eJobDefOf.IdleLovin, pawn, ownedBed);
                        pawn2.jobs.StartJob(newJob, JobCondition.InterruptForced);
                        return JobMaker.MakeJob(eJobDefOf.IdleLovin, pawn2, ownedBed);
        ...
        private static readonly SimpleCurve LovinIntervalHoursFromAgeCurve = new SimpleCurve
    {
        ...
        protected override IEnumerable<Toil> MakeNewToils()
        {
            ...
            {
                Map map = pawn.Map;
                if (Bed == null || !Bed.Spawned || Bed.Map != map || Partner == null || !Partner.Spawned /*|| Partner.Map != map*/)
                {
                    pawn.jobs.EndCurrentJob(JobCondition.Incompletable);
                    return;
                }
            ...
            gotoBed.AddPreTickIntervalAction(delegate (int delta)
            {
                if (pawn.IsHashIntervalTick(100, delta))
                {
                    ...
                    //    return;
                    //}
                    if ((Partner.CurJob == null || Partner.CurJob.def != eJobDefOf.IdleLovin) && Partner.CurJob.targetC != Bed)
                    {
                        pawn.jobs.EndCurrentJob(JobCondition.Incompletable);
                        return;
                    }
            ...
            gotoBed.socialMode = RandomSocialMode.Normal;
            ...
            toil1.AddPreTickIntervalAction(delegate (int delta)
            {
                ...
                {
                    Map map = pawn.Map;
                    if (Bed == null || !Bed.Spawned || Bed.Map != map || !Partner.Spawned || Partner.Map != map)
                    {
                        pawn.jobs.EndCurrentJob(JobCondition.Incompletable);
                        return;
                    }
                    ...
                    if (pawnPos.GetVacuum(map) >= 0.5f && pawn.GetStatValue(StatDefOf.VacuumResistance, true, 60) < 1f)
                    {
                        pawn.jobs.EndCurrentJob(JobCondition.Incompletable);
                        return;
                    }
                ...
                else if (pawn.IsHashIntervalTick(100, delta))
                {
                    ...
                    if (ticksLeftThisToil < 0 || Partner.health == null || !Partner.health.capacities.CanBeAwake || !LovePartnerRelationUtility.LovePartnerRelationExists(pawn, Partner))
                    {
                        ////Log.Warning("toil1e #3");
                        //if ((Partner.CurJob == null || Partner.CurJob.def != eJobDefOf.IdleLovin) && Partner.NextJobThinkResult() != null)
                        //{
                        //    //Log.Warning("toil1e #3t");
                        //    FleckMaker.ThrowMetaIcon(pawn.Position, map, FleckDefOf.Meditating);
                        //    return;
                        //}
                        ////Log.Warning("toil1e #3f");
                        pawn.jobs.EndCurrentJob(JobCondition.Incompletable);
                        return;
                    }
                    ...
                    if (!pawn.SafeTemperatureAtCell(pawnPos, map))
                    {
                        pawn.jobs.EndCurrentJob(JobCondition.Incompletable);
                        return;
                    }
                    ...
                    FleckMaker.ThrowMetaIcon(pawnPos, map, FleckDefOf.Meditating);
                }
            });
            ...
            toil2.AddPreTickIntervalAction(delegate (int delta)
            {
                ...
                else if (pawn.IsHashIntervalTick(100, delta))
                {
                    ...
                    FleckMaker.ThrowMetaIcon(pawnPos, map, FleckDefOf.Heart);
                }
            });
            toil2.AddFinishAction(delegate
            {
                ...
                if ((base.pawn.health.hediffSet.hediffs.Any((Hediff h) => h.def == HediffDefOf.LoveEnhancer)) || (Partner.health.hediffSet.hediffs.Any((Hediff h) => h.def == HediffDefOf.LoveEnhancer)))
                {
